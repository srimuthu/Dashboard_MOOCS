...
def convert_ondemand_main(path_to_files, postgres_database_name):
	# Get file names
	files = helpers(path_to_files).unique_files()
	# Create database
	psql = postgresql(postgres_database_name, config.postgres_user, config.postgres_pwd, config.postgres_host)
	psql.create_database()
	# Get sql statements for each file
	for file_ in files:
		if file_ == "readme" or file_ == "guide.pdf" or helpers(path_to_files).near_empty_files(file_) == True:
			continue
		# Initiate scraper and scrape
		scr = scraper(path_to_files, file_).scrape()
		print scr
		# Create SQL tables
		psql.insert_headers(scr)
		# Insert CSV data
		psql.insert_data(path_to_files, file_)
